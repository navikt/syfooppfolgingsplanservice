...
class BehandlendeEnhetConsumer(
    ...
    fun getBehandlendeEnhet(fnr: String): BehandlendeEnhet {
        metric.tellHendelse(METRIC_CALL_BEHANDLENDEENHET)
        ...
        try {
            ...
            metric.tellHendelse(METRIC_CALL_BEHANDLENDEENHET_SUCCESS)

            return responseBody!!
        } catch (e: HttpClientErrorException) {
            metric.tellHendelse(METRIC_CALL_BEHANDLENDEENHET_FAIL)
            throw e
        }
    }
    ...
    private fun authorizationHeader(token: String): HttpHeaders {
        val credentials = bearerHeader(token)
